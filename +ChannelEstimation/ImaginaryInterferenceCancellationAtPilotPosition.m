          ...
          switch Method
              case 'Auxiliary'                  
                  ...
                  NrAuxiliarySymbols    = sum(PilotMatrix(:)==-1);
                                                                                                % a->aux_pilots p->pilots d->data 
                  PseudoInvers      = pinv(FBMCMatrix(PilotMatrix(:)==1,PilotMatrix(:)==-1));% D(p,a)处的伪逆（即导频位置处对应D矩阵的行，辅助导频位置处对应D矩阵的列）
                  AuxMatrixPilots   = PseudoInvers*(eye(NrPilotSymbols)-FBMCMatrix(PilotMatrix(:)==1,PilotMatrix(:)==1));% AP = D(p,a)^-1 * (I(8*8)-D(p,p))
                  AuxMatrixData     = -PseudoInvers*FBMCMatrix(PilotMatrix(:)==1,PilotMatrix(:)==0);% AD = -D(p,a)^-1 * D(p,d)


                  AuxiliaryMatrix = zeros(numel(PilotMatrix),numel(PilotMatrix)-NrAuxiliarySymbols);% Aux数组的行数=导频矩阵元素的个数 列数=导频矩阵元素的个数-Aux导频的数量
                  AuxiliaryMatrix(PilotMatrix==-1,1:NrPilotSymbols)     = AuxMatrixPilots;% A(a,1:8) = AP
                  AuxiliaryMatrix(PilotMatrix==-1,NrPilotSymbols+1:end) = AuxMatrixData;   %A(a,9:352) = AD
                  AuxiliaryMatrix(PilotMatrix==1,1:NrPilotSymbols)      = eye(NrPilotSymbols)*sqrt(PilotToDataPowerOffset); % A(p,1:8) = I(8*8)*sqrt(kp,d)
                  AuxiliaryMatrix(PilotMatrix==0,NrPilotSymbols+1:end)  = eye(NrDataSymbols);% A(d,9:352) = I(344*344)
